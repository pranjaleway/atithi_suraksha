...
class APIHotelController extends Controller
{
     ...
     *     description="Deletes a hotel booking by ID. If the booking has associated members, they will also be deleted.",
     *     security={{"bearerAuth":{}}},
     *
     *     @OA\Parameter(
     *         name="id",
     *         in="query",
     *         required=true,
     *         description="ID of the hotel booking to delete",
     ...
     *         @OA\JsonContent(
     *             @OA\Property(property="status", type="string", example="success"),
     *             @OA\Property(property="message", type="string", example="Hotel Booking deleted successfully")
     *         )
     *     ),
     *
     *     @OA\Response(
    ...
    public function viewBookingDetails(Request $request)
    {
        try {
            ...
            if ($booking) {
                // Add booked_by field
                $booking->booked_by = $booking->hotel_employee_id ? 'Employee' : 'Owner';
    ...
    public function addMember(Request $request)
    {
        try {
            ...
            $mainBooking = HotelBooking::find($guestData['parent_id']);
            if ($mainBooking) {
                $mainBooking->update([
    ...
    public function getVisitors(Request $request)
    {
        ...
        $parentBooking = HotelBooking::find($bookingId);
        if ($parentBooking && $parentBooking->check_out <= now()) {
            $canAdd = false;
            $canDelete = false;
        }
